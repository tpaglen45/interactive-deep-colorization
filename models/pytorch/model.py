...
class SIGGRAPHGenerator(nn.Module):
    ...
    def forward(self, input_A, input_B, mask_B, maskcent=0):
        ...
        input_A = torch.tensor(input_A, dtype=torch.float32, device=device)[None, :, :, :]
        input_B = torch.tensor(input_B, dtype=torch.float32, device=device)[None, :, :, :]
        mask_B = torch.tensor(mask_B, dtype=torch.float32, device=device)[None, :, :, :]
